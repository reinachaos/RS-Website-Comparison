...
Headings are limited to rendered visible headings, including the first visible page H1. `imagesSettled` requires loaded rendered main-content images, not merely a finished network attempt; broken images do not qualify. Native lazy images are requested before extraction. Custom JavaScript lazy-loading states remain outside this readiness guarantee. A return-to-top activation must start at an observed non-top position, so focus-induced scrolling cannot certify a nonfunctional link.

Evidence redaction sanitizes credential URL components and removes/masks sensitive form controls. Known form/query/fragment values of at least eight characters are also masked in text and attribute values before DOM serialization. Short values are not replaced throughout ordinary article text because they can collide with public letters or years. Long secret collisions can intentionally redact matching public text; no claim of comprehensive secret detection is made. Review evidence before sharing. Lazy images without intrinsic dimensions are promoted before dimension filtering; zero-size loading boxes conservatively wait for completion rather than proving that imagery is absent.
...
Execution failures preserve completed supplemental records and emit per-item error records without aborting unrelated items. If launch or a broader failure prevents execution, every planned missing ID is emitted with a `not-run` blocked check. `coverage.notRunIds` lists them; `availabilityExecuted`, `filePairsExecuted` and `anchorsExecuted` exclude such placeholders even though they appear in reported totals. `COVERAGE-INCOMPLETE` forces an error gate. Browser cleanup failure likewise becomes an explicit tool result and does not suppress the saved report.

Partial HTTP representations (206 or Content-Range) do not establish complete file bytes. Pending or unresolved redirect page statuses cannot satisfy content predicates. A browser-readable fallback page cannot certify an expected download when complete document evidence is absent; an empty body without a loaded visible image is blocked. External-link collector exceptions retain preceding finding checks and do not skip later external destinations. Catalog context and report corrections are explicitly historical/contextual notes, not fresh defect confirmations.
